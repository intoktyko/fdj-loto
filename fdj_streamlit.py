...
st.set_page_config(page_title="Generator FDJ", page_icon="🎰")
...
st.markdown("<h1 style='text-align:center;color:#e94560;'>🎰 GENERATOR FDJ</h1>", unsafe_allow_html=True)
st.markdown("<p style='text-align:center;color:gray;'>Loto · EuroMillions · Keno</p>", unsafe_allow_html=True)
st.markdown("---")
col1, col2 = st.columns(2)
with col1:
    joc = st.selectbox("🎮 Joc", ["Loto 6/49", "EuroMillions", "Keno 20/70"])
with col2:
    strat = st.selectbox("🧠 Strategie", ["Mixta (IA)", "Frecventa", "Retardataire", "Aleatorie"])
if st.button("✨ GENEREAZA NUMERE", use_container_width=True, type="primary"):
    st.markdown("---")
    if joc == "Loto 6/49":
        numere, chance = gen_loto(strat)
        st.success(f"🎱 LOTO 6/49: {numere} + Chance: {chance}")
        st.session_state.istoric.insert(0, f"Loto | {numere} + Chance:{chance} | {strat}")
    elif joc == "EuroMillions":
        numere, stele = gen_euro(strat)
        st.success(f"⭐ EUROMILLIONS: {numere} + Stele: {stele}")
        st.session_state.istoric.insert(0, f"Euro | {numere} + Stele:{stele} | {strat}")
    else:
        numere = gen_keno()
        st.success(f"🎰 KENO: {numere}")
        st.session_state.istoric.insert(0, f"Keno | {numere}")
st.markdown("---")
st.markdown("**📋 Istoric:**")
if st.session_state.istoric:
    for i, item in enumerate(st.session_state.istoric[:10]):
        st.markdown(f"`#{i+1}` {item}")
else:
    st.markdown("*Nicio generare inca...*")
